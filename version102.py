...
while True: # keep the program running, allowing constant reading of tags without restarting
	# create variables
	inside = "in"
	outside = "out"
	...
	while direction != outside and direction != inside: # if direction hasn't been determined yet, ask for it
	
		direction = input("Going inside or outside?: ")
	
	reader = SimpleMFRC522.SimpleMFRC522()
	id, text = reader.read() # read the name on a tag
	...
	if text.replace(" ", "") in wb.sheetnames: # if sheet with name already exists, use it
		nameis = "True"
		sheet = wb[text.replace(" ", "")]
	
	if nameis != "True": # if sheet with name doesn't exist, create one and use it
		wb.create_sheet(text.replace(" ", ""))
		sheet = wb [text.replace(" ", "")]

	if direction == inside: # if heading to work, write all the data on columns 1-4
	        a = sheet.max_row
		sheet.cell(row=a+1,column=1).value="Inside"
	        ...
	        wb.save(text.replace(" ","") + '.xlsx') # save the file and copy it to NextCloud client folder
		shutil.copy2('/home/pi/pilvijarjestelma/' + text.replace(" ", "") + '.xlsx', '/home/pi/NextcloudFold/excel')
		print("Welcome, " + text)
	
		GPIO.cleanup()
	
	elif direction == outside: # if leaving work, write on columns 5-8
		a = sheet.max_row
		...
		sheet.cell(row=a+1,column=10).value='=sum()' # calculate work time -- code missing
